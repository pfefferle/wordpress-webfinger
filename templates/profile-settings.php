		...
		<tr>
			...
			<td>
				<p>
					...
					<input name="webfinger_resource" id="webfinger_resource" type="text" style="text-align: right;" value="<?php echo esc_attr( get_webfinger_username( $args['user']->ID ) ); ?>" />@<?php echo esc_html( parse_url( home_url(), PHP_URL_HOST ) ); ?>
				</p>
				<p class="description">
			...
			<td>
				...
				<?php foreach ( \Webfinger\User::get_resources( $args['user']->ID ) as $resource ) { ?>
					<li>
						...
						<small>(<a href="https://webfinger.net/lookup/?resource=<?php echo esc_url( urlencode( $resource ) ); ?>" target="_blank" rel="noopener noreferrer">verify</a>)</small>
					</li>
				<?php } ?>
				</ul>
			</td>
